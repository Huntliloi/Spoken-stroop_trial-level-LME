...
The repository follows the abbreviated condition labels used in the current
manuscript:

| Label | Stroop condition |
|---|---|
| `W` | Word Reading |
| `C` | Color Naming |
| `CW` | Color-Word Interference |
...
accuracy and total hesitation duration for W, C, and CW, together with
...
three automated Age x Task contrast tables underlying the primary analyses and
Tables S4-S6. The primary LME script is
...
The 90-feature sensitivity analysis controlling task-specific accuracy and
card completion time is provided as:

- `scripts/04_age_related_acoustic_analysis/Z.1_90_feature_traditional_score_adjusted_LME.py`
- `results/02_age_related_acoustic_responses/traditional_score_adjusted_sensitivity/`

After adjustment, 24 of 90 CW-versus-W Age x Task effects remained significant
at `q <= .05`, including 17 at `q <= .01`; 85 effects retained the original
direction, and the original and adjusted coefficients correlated at `r = .970`.
Of the 24 significant adjusted effects, 23 were significant in the primary
model and one was newly significant (VoicedSegmentsPerSec). The median
coefficient attenuation was 14.1%. Original-model coefficients and p values
are read directly from the primary CW-versus-W table underlying Table S4,
without refitting. These results correspond to Table S8.
...
The overall feature-level repeatability and cross-device summaries are:

- `results/03_repeatability_cross_device/table2_within_device_retest_summary.csv`
- `results/03_repeatability_cross_device/table_s10_cross_device_summary.csv`
- `results/03_repeatability_cross_device/table_s11_cross_device_features_icc_ge_0_50.csv`

The supplementary comparison of traditional measures, condition-specific
acoustic levels, and model-based individual CW-versus-W task slopes is under:

- `scripts/05_repeatability_cross_device/04_task_specific_retest_reliability.py`
- `results/03_repeatability_cross_device/task_specific_reliability/`

The compact result is `table_s9_task_specific_reliability.csv`. All three
devices are analyzed separately. Overall acoustic levels showed moderate
repeatability, whereas person-specific CW-versus-W task effects were much less
stable. These results correspond to Table S9.

Age-referenced acoustic deviation indices were also evaluated across the two
sessions. The analysis uses the same 25 prespecified features as the external
clinical evaluation and reports ICC(3,1) separately for W, CW, and CW-minus-W
Mean |Z_dev| and RMS Z_dev indices:

- `scripts/05_repeatability_cross_device/06_deviation_index_retest_reliability.py`
- `results/03_repeatability_cross_device/table_s12_deviation_index_retest_reliability.csv`

This compact summary corresponds to Table S12. The script also regenerates
the detailed, anonymous session-level and feature-level outputs under
`outputs/`.
...
`data/public/wd/` contains 12 WD-CI and 31 WD-CN participants. The same 25
prespecified features are used to calculate `Mean |Z_dev|` and `RMS Z_dev`
for W, CW, and the model-based CW-minus-W task effect. The public results are:

- `results/04_wd_external_validation/table3_wd_deviation_indices_by_condition.csv`
- `results/04_wd_external_validation/wd_ci_vs_wd_cn_statistics.csv`
- `results/04_wd_external_validation/wd_subject_condition_deviation_indices.csv`

The last file is the anonymous subject-level source for Figure 6. Script 01
reproduces the deviation scores, Table 3, and Figure 6. Script 02 documents
feature recovery from restricted recordings.

### 6. External Evaluation in the Substance-Use Cohort

`data/public/substance_use/` contains 28 Drug participants and 30 healthy
controls. The same W, CW, and CW-minus-W acoustic deviation measures are used.
Public statistical outputs are:

- `results/05_substance_use_external_validation/table4_moca_correlations_by_condition.csv`
- `results/05_substance_use_external_validation/table_s13_moca_groups_by_condition.csv`
- `results/05_substance_use_external_validation/table_s14_drug_vs_hc_by_condition.csv`
- `results/05_substance_use_external_validation/subject_condition_deviation_indices.csv`
...
Spearman correlations in Table 4 use uncorrected MoCA scores and are corrected
across the six condition-by-index correlations. The subject-level file is the
source for Figure 7. Script 04 reproduces the scores, Tables 4/S13/S14, and
Figure 7. Script 03 documents feature recovery from restricted recordings.

### 7. Comparison with Traditional Stroop Scoring

The final clinical comparison uses six age-referenced traditional Stroop
measures: W and CW accuracy, W and CW card completion time, and the
CW-versus-W interference costs for accuracy and completion time. These
measures are compared with six acoustic measures: `Mean |Z_dev|` and
`RMS Z_dev` for W, CW, and CW-minus-W. The combined model therefore contains
12 predictors. Predictions are generated with leave-one-subject-out
cross-validated L2 logistic regression; median imputation and standardization
are fitted within each training fold, and AUC confidence intervals use
DeLong's method.
...
- `results/06_behavioral_acoustic_comparison/table5_behavioral_acoustic_auc.csv`

The cohort-specific directories contain the six anonymous acoustic model
inputs and complete cross-validated AUC outputs. These compact frozen inputs
reproduce the values reported in Table 5 exactly; the full feature-level and
model-fitting intermediates can be regenerated from the public data.
...
python scripts/04_age_related_acoustic_analysis/Z.1_90_feature_traditional_score_adjusted_LME.py
python scripts/05_repeatability_cross_device/04_task_specific_retest_reliability.py
python scripts/05_repeatability_cross_device/06_deviation_index_retest_reliability.py
python scripts/06_external_validation/01_wd_deviation_scores.py
python scripts/06_external_validation/04_substance_use_deviation_scores.py
