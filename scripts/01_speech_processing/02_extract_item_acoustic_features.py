...
TASK_COLS = ["task", "Task"]
ITEM_COLS = ["token_index", "Item_Index", "idx"]
...
def find_audio_file(subject_id: str, task_report_name: str) -> Optional[str]:
    ...
    xxxxxxxx001_stroop_C.wav
    xxxxxxxx001_stroop_CW.wav
    xxxxxxxx001_stroop_W.wav
    """
...
def prepare_timestamp_df() -> pd.DataFrame:
    print('\n--- Step 1/3: derive item duration and Gap_Before from token_results.csv ---')
    ...
    if missing:
        raise ValueError(f"token_results.csv is missing required columns: {missing}")
    ...
    print(f"Automatic timestamp collection complete: {len(work)} rows.")
    if not work.empty:
        print('Rows by task:')
...
def extract_opensmile_features_in_memory(df_timestamps: pd.DataFrame) -> pd.DataFrame:
    print('\n--- Step 2/3: extract item-level OpenSMILE features in memory ---')
    print('OpenSMILE feature set: eGeMAPSv02 (Functionals)')
    ...
    if not test_done or not feature_columns:
        print('Error: no valid audio segment was available to initialize OpenSMILE features.')
    ...
    for _, row in tqdm(df_timestamps.iterrows(), total=len(df_timestamps), desc='OpenSMILE extraction'):
        sid = str(row["Subject_ID"])
        task = row["Task"]
        ...
        if not audio_path or not os.path.exists(audio_path):
            print(f"Warning: audio not found for Subject={sid}, Task={task}")
        ...
        try:
            ...
            if len(seg) == 0:
                print(f"Warning: empty audio slice for Subject={sid}, Task={task}, Item={item_idx}")
                for col in feature_columns:
                    feature_row[col] = np.nan
                all_features_data.append(feature_row)
                continue
            ...
            if not features_df_single.empty:
                ...
                print(f"Warning: feature extraction returned no output for Subject={sid}, Task={task}, Item={item_idx}")
                for col in feature_columns:
                    feature_row[col] = np.nan

        except Exception as e:
            print(f"Error processing Subject={sid}, Task={task}, Item={item_idx}: {e}")
            for col in feature_columns:
                feature_row[col] = np.nan
...
def merge_and_save(df_timestamps: pd.DataFrame, features_df: pd.DataFrame):
    print('\n--- Step 3/3: merge timestamps and acoustic features ---')
    ...
    if opensmile_cols:
        ...
        print(f"OpenSMILE missing values: {missing_values} / {total_cells} ({missing_percentage:.1f}%)")
...
if __name__ == "__main__":
    try:
        ...
        print('OpenSMILE feature set: eGeMAPSv02')
        print('OpenSMILE feature level: Functionals')
    except ImportError:
        print('Error: opensmile-python is required. Install it with: pip install opensmile')
        raise
