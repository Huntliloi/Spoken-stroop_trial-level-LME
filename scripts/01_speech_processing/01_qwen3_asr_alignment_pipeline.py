...
try:
    ...
    raise ImportError('PyTorch is required. Install a compatible torch build.') from e

try:
    import soundfile as sf
except Exception as e:
    raise ImportError('soundfile is required. Install it with: pip install soundfile') from e

try:
    from qwen_asr import Qwen3ASRModel
except Exception as e:
    raise ImportError('qwen_asr is required. Install the qwen-asr package.') from e
...
def normalize_device(device: str) -> str:
    if device.startswith("cuda") and not torch.cuda.is_available():
        print('CUDA is unavailable in PyTorch; falling back to CPU.')
        return "cpu"
    return device
...
def detect_task_from_filename(filename: str) -> Optional[str]:
    ...
    if "_stroop_cw" in name or name.endswith("cw.wav"):
        return "CW"
    if "_stroop_w" in name or name.endswith("w.wav"):
        return "W"
    if "_stroop_c" in name or name.endswith("c.wav"):
        return "C"
...
def find_audio_files(audio_root: str) -> List[Path]:
    """Collect public-example Stroop wav files for W, C, and CW tasks.
...
def clean_color_sequence(raw_tokens: List[str], expected_n: int = EXPECTED_TOKENS_PER_TASK) -> Tuple[List[str], str]:
    """Remove fillers, retain color tokens, and enforce the expected item count."""
...
def align_cleaned_text_if_needed(model, audio_path: str, cleaned_colors: List[str]) -> List[Dict[str, Any]]:
    """Run forced alignment when ASR token timestamps are incomplete."""
...
def refine_one_pair_boundary(
    ...
    """Refine the boundary between two adjacent item tokens."""
...
def _compute_speech_score_segment(
    ...
    """Combine robust energy, envelope, and fluctuation cues into a speech score."""
...
def refine_first_token_start(
    ...
    """Refine the first item onset within a constrained pre-roll window."""
...
def refine_last_token_end(
    ...
    """Refine the final item offset within a constrained post-roll window."""
...
def refine_boundaries_by_pairwise_features(
    ...
    """Refine internal boundaries, then adjust the first onset and final offset."""
