"""Compare traditional Stroop and acoustic deviations in the WD cohort.

The traditional benchmark uses W and CW accuracy, card completion time,
and CW-vs-W interference costs. Each measure is converted to an
...
DEFAULT_ACOUSTIC_SCORE_CSV = (
    REPO_ROOT
    / "results"
    / "06_behavioral_acoustic_comparison"
    / "wd"
    / "acoustic_model_inputs.csv"
)
...
EXPECTED_ITEMS = 12
TASK_TARGETS = {
    "W": ["yellow", "red", "green", "blue", "green", "yellow", "blue", "red", "blue", "green", "red", "yellow"],
    "CW": ["yellow", "red", "green", "blue", "green", "blue", "yellow", "red", "red", "blue", "green", "yellow"],
}
COLOR_LABELS = {"\u9ec4": "yellow", "\u7ea2": "red", "\u7eff": "green", "\u84dd": "blue"}

ACOUSTIC_COLUMNS = [
    "W__Mean_Abs_Deviation_Z",
    "W__RMS_Deviation_Z",
    "CW__Mean_Abs_Deviation_Z",
    "CW__RMS_Deviation_Z",
    "CW_minus_W__Mean_Abs_Deviation_Z",
    "CW_minus_W__RMS_Deviation_Z",
]
TASK_LEVEL_TRADITIONAL_COLUMNS = [
    "Overall_Accuracy_W",
    "Card_Completion_Time_W",
    "Overall_Accuracy_CW",
    "Card_Completion_Time_CW",
]
INTERFERENCE_COLUMNS = [
    "Accuracy_Cost_CW_vs_W",
    "Completion_Time_Cost_CW_vs_W",
]
...
MEASURES = [
    ("W accuracy Z_dev", TRADITIONAL_Z_COLUMNS[0], "Traditional Stroop", "Lower is worse"),
    ("W completion-time Z_dev", TRADITIONAL_Z_COLUMNS[1], "Traditional Stroop", "Higher is worse"),
    ("CW accuracy Z_dev", TRADITIONAL_Z_COLUMNS[2], "Traditional Stroop", "Lower is worse"),
    ("CW completion-time Z_dev", TRADITIONAL_Z_COLUMNS[3], "Traditional Stroop", "Higher is worse"),
    ("Accuracy-interference Z_dev: CW vs W", TRADITIONAL_Z_COLUMNS[4], "Traditional Stroop", "Higher is worse"),
    ("Completion-time-interference Z_dev: CW vs W", TRADITIONAL_Z_COLUMNS[5], "Traditional Stroop", "Higher is worse"),
    ("Mean |Z_dev|: W", ACOUSTIC_COLUMNS[0], "Acoustic", "Higher is worse"),
    ("RMS Z_dev: W", ACOUSTIC_COLUMNS[1], "Acoustic", "Higher is worse"),
    ("Mean |Z_dev|: CW", ACOUSTIC_COLUMNS[2], "Acoustic", "Higher is worse"),
    ("RMS Z_dev: CW", ACOUSTIC_COLUMNS[3], "Acoustic", "Higher is worse"),
    ("Mean |Z_dev|: CW-W", ACOUSTIC_COLUMNS[4], "Acoustic", "Higher is worse"),
    ("RMS Z_dev: CW-W", ACOUSTIC_COLUMNS[5], "Acoustic", "Higher is worse"),
]
MODEL_SETS = {
    "Traditional_Stroop": TRADITIONAL_Z_COLUMNS,
    "Expanded_Acoustic": ACOUSTIC_COLUMNS,
    "Combined_Traditional_Expanded_Acoustic": TRADITIONAL_Z_COLUMNS + ACOUSTIC_COLUMNS,
}
...
def load_acoustic_scores(score_path: Path) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"Acoustic score table is missing columns: {sorted(missing)}")
    return scores[["Subject_ID", "Clinical_Group", "Age"] + ACOUSTIC_COLUMNS]
...
def build_subject_behavior(task_metrics: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"Cannot construct CW-W contrasts; missing columns: {missing}")
    wide["Accuracy_Cost_CW_vs_W"] = wide["Overall_Accuracy_W"] - wide["Overall_Accuracy_CW"]
    wide["Completion_Time_Cost_CW_vs_W"] = (
        wide["Card_Completion_Time_CW"] - wide["Card_Completion_Time_W"]
    )
    for task in ["W", "CW"]:
        wide[f"Complete_12_Items_{task}"] = wide[f"Valid_Item_Count_{task}"].eq(EXPECTED_ITEMS)
    wide["QC_W_CW_Complete_12"] = wide[
        ["Complete_12_Items_W", "Complete_12_Items_CW"]
    ].all(axis=1)
    return wide
...
def load_healthy_reference(behavior_csv: Path, feature_csv: Path, info_xlsx: Path) -> pd.DataFrame:
    ...
    task_metrics = task_metrics[task_metrics["Task"].isin(["W", "CW"])].copy()
...
def load_wd_data(
    healthy_behavior_csv: Path,
    healthy_feature_csv: Path,
    healthy_info_xlsx: Path,
    acoustic_score_csv: Path,
) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    item_frames = []
    specifications = [
        ("WD-CI", PUBLIC_WD_FEATURE_DIR / "Stroop_features_WD_CI.csv"),
        ("WD-CN", PUBLIC_WD_FEATURE_DIR / "Stroop_features_WD_CN.csv"),
    ]
    for group, item_path in specifications:
    ...
    scores = load_acoustic_scores(acoustic_score_csv)
...
def univariate_group_comparison(subject_data: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for analysis_set, subset in [("Full sample", subject_data)]:
...
1. Overall accuracy is the number of correctly paired recognized color words divided by 12. Missing recognized items therefore count as incorrect. Accuracy is calculated separately for W and CW.
...
3. CW-vs-W accuracy interference is W accuracy minus CW accuracy. CW-vs-W completion-time interference is CW completion time minus W completion time. Higher values indicate greater interference.
...
5. The traditional Stroop model uses six signed age-referenced Z deviations: W accuracy, W completion time, CW accuracy, CW completion time, and the two CW-vs-W interference costs.
...
7. BH correction is applied separately across the six traditional Stroop tests and the six acoustic tests.
...
9. Model AUCs use leave-one-subject-out predictions from L2-penalized logistic regression. Median imputation and standardization are fitted within each training fold.
10. The combined model evaluates the complementary information in traditional Stroop and acoustic deviations; it is not an independently validated diagnostic model.
11. Missing recognized items count as errors, and missing completion-time predictors are median-imputed within each training fold.
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--acoustic-score-csv", type=Path, default=DEFAULT_ACOUSTIC_SCORE_CSV
    )
    ...
    with warnings.catch_warnings():
        ...
        _, subject_data, reference = load_wd_data(
            ...
            args.acoustic_score_csv.resolve(),
        )
    ...
    outputs = {
        ...
        "subject_level_behavioral_and_expanded_acoustic_deviations.csv": subject_data,
