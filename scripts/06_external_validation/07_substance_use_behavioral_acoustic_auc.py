"""Compare age-referenced behavioral and acoustic deviations in substance use.
...
10. Model AUCs use leave-one-subject-out predictions from L2-penalized logistic regression. The combined model evaluates whether acoustic deviations add useful information to conventional behavioral deviations; it is not an independently validated diagnostic model.
11. Full-sample analyses count missing recognized items as errors. Complete-case sensitivity analyses include only participants with 12 recognized items in all three tasks; ASR substitutions can still be indistinguishable from true response errors without manual annotation.
"""
