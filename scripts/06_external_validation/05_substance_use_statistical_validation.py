"""Reproduce substance-use cohort Tables 6, S9, and S10."""
...
def build_table_s10(frame: pd.DataFrame) -> pd.DataFrame:
    parts = []
...
def build_table6(frame: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for measure, column in MEASURES.items():
        row = {"Measure": measure}
        for analysis_set, subset in [
...
def main() -> None:
    ...
    outputs = {
        "table_s9_drug_vs_hc.csv": compare_groups(frame, "Group", "Drug", "HC"),
        "table_s10_moca_groups.csv": build_table_s10(frame),
        "table6_moca_correlations.csv": build_table6(frame),
    }
