...
def build_table_s13(frame: pd.DataFrame) -> pd.DataFrame:
    parts = []
...
def build_table4(frame: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for measure, column in MEASURES.items():
        row = {"Measure": measure}
        for analysis_set, subset in [
...
def main() -> None:
    ...
    outputs = {
        "table_s12_drug_vs_hc.csv": compare_groups(frame, "Group", "Drug", "HC"),
        "table_s13_moca_groups.csv": build_table_s13(frame),
        "table4_moca_correlations.csv": build_table4(frame),
    }
