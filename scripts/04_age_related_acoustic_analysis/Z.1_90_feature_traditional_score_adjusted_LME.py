...
Original-model estimates are read directly from the primary CW-versus-W
analysis underlying Table S4; they are not refitted in this analysis.
"""
...
def load_original_results(path: Path) -> pd.DataFrame:
    if not path.exists():
        raise FileNotFoundError(path)
    original = pd.read_csv(path, encoding="utf-8-sig", float_precision="round_trip")
    required = {
        "Feature",
        "Interaction_Coef",
        "Interaction_PValue",
    ...
    return original[
        ["Feature", "Interaction_Coef", "Interaction_PValue", "Interaction_FDR_PValue"]
    ].rename(
        columns={
            "Interaction_Coef": "Original_Age_Task_Beta",
...
10. Original-model coefficients and p values are taken directly from the primary CW-versus-W analysis underlying Table S4, not from a separately fitted clinical healthy-reference model.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Reproduce the 90-feature traditional-score-adjusted LME analysis."
    )
