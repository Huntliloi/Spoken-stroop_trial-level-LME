'\nCompute WD deviation scores from healthy LME norms for STT_vs_Hanzi only.\n\n     SCORING_GROUP：\n    "WD_CI"  :   12 WD    \n    "WD_HC"  :     WD\n    "WD_ALL" :       \n\n            LME   Age_Z x Task_Code   FDR     p < 0.01    。\n'
...
# =========================
# =========================
SCORING_GROUP = "WD_CI"  # options: "WD_CI", "WD_HC", "WD_ALL"


# =========================
# Paths and parameters
# =========================
BASE_DIR = OUTPUT_DIR / "wd_external_validation_not_included"  # noqa: F405

HEALTHY_FEATURES_PATH = get_imputed_acoustic_features_csv()  # noqa: F405
HEALTHY_INFO_PATH = PARTICIPANT_INFO_XLSX  # noqa: F405
WD_INFO_PATH = BASE_DIR / "wd_clinical_info_not_included.xlsx"

WD_FEATURE_PATHS = {
    "WD_CI": BASE_DIR / "Stroop_features_WD_add.csv",
    "WD_HC": BASE_DIR / "Stroop_features_WD_HC_add.csv",
}
...
META_COLS = {
    ...
    "Correct", "Correctness",
    "Age", "Age_Z", "Sex", "Gender", "Education", "MMSE",
...
def load_healthy_info():
    ...
    subject_col = find_col(info, ["Subject_ID", "Subject", "ID", '  Subject_ID', "Subject_ID"])
    age_col = find_col(info, ["Age", "Age"])
    sex_col = find_col(info, ["Sex", "Gender", "Sex"])
    edu_col = find_col(info, ["Education", "Education", 'English documentation.', 'English documentation.', ' Education'])
...
def load_wd_info():
    ...
    subject_col = find_col(info, ["Subject_ID", "Subject", "ID", '  Subject_ID', "Subject_ID", 'English documentation.'])
    if subject_col is None:
        return pd.DataFrame(columns=["Subject_ID"])
    ...
    for names, target in [
        ...
        (["Sex", "Sex", "Gender"], "Sex"),
        (["Age", "Age"], "Age"),
        (["Education", 'English documentation.', 'English documentation.', ' Education', "Education"], "Education"),
        (["MMSE", 'MMSE  ', 'MMSE  '], "MMSE"),
        (["Frenchay", "FDA", 'Frenchay  '], "Frenchay"),
    ]:
...
def load_wd_features():
    if SCORING_GROUP not in {"WD_CI", "WD_HC", "WD_ALL"}:
        raise ValueError('SCORING_GROUP must be "WD_CI", "WD_HC", or "WD_ALL".')

    frames = []
    groups = ["WD_CI", "WD_HC"] if SCORING_GROUP == "WD_ALL" else [SCORING_GROUP]
...
def prepare_data():
    ...
    for col in ["Age", "Sex", "Education", "MMSE", "Frenchay", "Name"]:
...
def main():
    ...
    wd_meta_cols = [
        c for c in ["Subject_ID", "Cognitive_Group", "Name", "Sex", "Age", "Education", "MMSE", "Frenchay"]
        if c in wd.columns
    ]
