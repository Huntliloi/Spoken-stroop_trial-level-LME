...
Main analysis:
    For each device separately:
        ...
        overall mean across all Stroop task items in session_1 vs session_2
...
def write_notes(path: Path, feature_cols: List[str], overall_table: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        "  Each feature is first averaged across all Stroop task items for the same Subject_ID x Device_ID x Session.",
        "  ICC is computed within each device for session_1 vs session_2.",
        "",
        "No item-level or task-level ICC is computed in this script.",
        "Reason: item order/content can differ across Stroop task sequences, so strict item-index matching is not appropriate.",
        "",
